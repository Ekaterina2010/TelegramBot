...
pict = ['https://avatars.mds.yandex.net/get-pdb/2864819/2091b635-1a05-4a81-9f4f-9cdd46cb9be0/s1200',
        'https://avatars.mds.yandex.net/get-zen_doc/196516/pub_5d65e93efe289100adb4c54e_5d66099378125e00ac052d00'
        '/scale_1200',
        'https://avatars.mds.yandex.net/get-pdb/1683100/d71b5f09-b408-42ce-b480-cbcd0d340efe/s1200?webp=false',
        'https://avatars.mds.yandex.net/get-zen_doc/1899089/pub_5d9b5f2f35c8d800ae71fb5a_5d9b60a98f011100b48eb4fb'
        '/scale_1200',
        'https://avatars.mds.yandex.net/get-zen_doc/196516/pub_5d65e93efe289100adb4c54e_5d66099378125e00ac052d00'
        '/scale_1200',
...
col1.append(rows[0].find_all('th')[1].get_text().strip())
...
print(col1)
col2 = [rows[0].find_all('th')[2].get_text().strip()]
...
field_list = []
for i in range(9):
    col = [rows[0].find_all('th')[i + 1].get_text().strip()]
    for row in rows[1:]:  # начинаем со второго ряда таблицы, потому что 0 уже обработали выше
        r = row.find_all('td')  # находим все теги td для строки таблицы
...
def cmd_info(message):
    ...
    bot.send_message(message.chat.id, "The next step is to specify which countries/regions you are interested in.\n"
                                      "You should enter comma-delimited list of countries or regions or just a single "
                                      "country/region:\n "
...
@bot.message_handler(func=lambda message: dbworker.get_current_state(
    message.chat.id) == config.States.S_COUNTRY_OR_REGION.value and message.text.strip().lower() not in (
                                          '/reset', '/info', '/start', '/commands', '/listregions', '/listfields'))
...
@bot.message_handler(
    func=lambda message: dbworker.get_current_state(message.chat.id) == config.States.S_ENTER_COUNTRY_OR_REGION.value
...
@bot.message_handler(
    func=lambda message: dbworker.get_current_state(
        message.chat.id) == config.States.S_ENTER_FIELD_LIST.value and message.text.strip().lower() not in (
                         '/reset', '/info', '/start', '/commands', 'listregions', '/listfields'))
...
@bot.message_handler(func=lambda message: message.text not in ('/reset', '/info', '/start', '/commands', '/listregions',
                                                               '/listfields'))
